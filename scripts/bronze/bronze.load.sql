create or alter procedure bronze.load_bronze as -- Создание процедуры
begin
	declare @start_time datetime, @end_time datetime, @start_time1 datetime, @end_time1 datetime
begin try
set @start_time1= GETDATE() -- Создание переменных чтобы выводить в принтах
		print '========================================'
		...
		print '----------------------------------------'
		--Очистка таблицы перед загрузкой в таблицу
		set @start_time = GETDATE();
		Print '>>>Truncation table crm_cust_info'
		truncate table bronze.crm_cust_info;
		Print '>>>Bulk inserting to table crm_cust_info'
		bulk insert bronze.crm_cust_info --Загрузка csv файла на таблицу с директории моего компа
		...
		bulk insert bronze.erp_PX_CAT_G1V2 --Загрузка csv файла на таблицу с директории моего компа
	...
	begin catch 
		...
		Print '==================================='
	end catch --Это сообщение выйдет если произойдет ошибка
